...
import { authApi } from "../services/api"; // <-- TAMBAHKAN IMPORT INI
...
const Sidebar: React.FC<SidebarProps> = ({ onQuickAdd }) => {
  ...
  const handleLogout = async () => {
    try {
      ...
      console.error(
        "Gagal logout di server, tapi token lokal akan tetap dihapus.",
        error,
      );
    } finally {
  ...
  const renderBottomCard = () => {
    if (location.pathname === "/") {
      return (
        <div
          className={`p-4 rounded-2xl border ${dark ? "bg-indigo-950/50 border-indigo-900" : "bg-indigo-50/50 border-indigo-100"}`}
        >
          <h4
            className={`flex items-center gap-2 font-bold mb-3 text-sm ${dark ? "text-indigo-300" : "text-indigo-800"}`}
          >
    ...
    let icon = (
      <ClipboardList
        size={32}
        className={`mb-2 opacity-50 ${dark ? "text-indigo-400" : "text-indigo-400"}`}
      />
    );
    ...
    return (
      <div
        className={`p-6 rounded-2xl border flex flex-col items-center text-center ${dark ? "bg-indigo-950/50 border-indigo-900" : "bg-indigo-50/50 border-indigo-100"}`}
      >
        <div
          className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 border ${dark ? "bg-indigo-900/50 border-indigo-800" : "bg-indigo-100/50 border-indigo-100"}`}
        >
          {icon}
        </div>
        <p
          className={`text-xs font-medium leading-relaxed ${dark ? "text-slate-400" : "text-slate-600"}`}
        >
          {text}
        </p>
      </div>
  ...
  return (
    <div
      className={`w-64 h-screen border-r flex flex-col fixed left-0 top-0 transition-colors duration-300 ${dark ? "bg-slate-800 border-slate-700" : "bg-white border-slate-100"}`}
    >
      {/* Logo Area */}
      <div
        className={`p-6 flex items-center gap-3 font-bold text-xl ${dark ? "text-slate-100" : "text-slate-800"}`}
      >
      ...
      <div className="flex-1 px-4 space-y-1 mt-2 overflow-y-auto">
        ...
        <div
          className={`my-4 border-t ${dark ? "border-slate-700" : "border-slate-100"}`}
        ></div>
      ...
      <div className="p-4 space-y-4">
        ...
        <button
          ...
          className="flex items-center gap-3 px-4 py-3 text-red-500 font-bold hover:bg-red-50 w-full rounded-xl transition-colors dark:hover:bg-red-950/30"
        >
          <LogOut size={20} />
          {t("logout")}
        </button>
