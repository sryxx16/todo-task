...
const PrioritasTable = ({ tasks, onView, onEdit, onDelete }: { tasks: Task[]; onView: (t:Task)=>void; onEdit: (t:Task)=>void; onDelete: (t:Task)=>void }) => (
  tasks.length === 0 ? <p className="text-slate-400 text-sm py-4 text-center">Tidak ada tugas</p> : (
        ...
        {tasks.map(t => (
          <tr key={t.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
            ...
            <td className="py-3">
              ...
              <p className="text-xs text-slate-500 truncate max-w-[160px]">{t.course?.name}</p>
            </td>
            <td className="py-3 text-slate-600">{t.course?.name}</td>
            <td className={`py-3 font-semibold ${new Date(t.deadline) < new Date() && t.status !== 'Selesai' ? 'text-red-500' : 'text-slate-600'}`}>{formatDate(t.deadline)}</td>
            ...
            <td className="py-3 w-32">
              <div className="flex items-center gap-2 text-xs">
                <span className="font-medium w-8">{t.progress}%</span>
                <div className="flex-1 h-1.5 bg-slate-100 rounded-full"><div className="h-full bg-indigo-600 rounded-full" style={{width:`${t.progress}%`}}></div></div>
              </div>
            </td>
            <td className="py-3">
              <div className="flex justify-center gap-1">
                <button onClick={()=>onView(t)} className="p-1.5 text-slate-400 hover:text-indigo-600 rounded-lg"><Eye size={15}/></button>
                <button onClick={()=>onEdit(t)} className="p-1.5 text-slate-400 hover:text-blue-600 rounded-lg"><Edit size={15}/></button>
                <button onClick={()=>onDelete(t)} className="p-1.5 text-slate-400 hover:text-red-600 rounded-lg"><Trash2 size={15}/></button>
              </div>
            </td>
          </tr>
        ))}
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={c.label} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
            ...
            <p className="text-xs text-slate-500 mb-3">{c.pct}% dari total tugas</p>
            <div className="w-full h-1 bg-slate-100 rounded-full"><div className={`h-full ${c.bar} rounded-full`} style={{width:`${c.pct}%`}}></div></div>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="flex-1 space-y-5">
          {/* Controls */}
          <div className="flex justify-between items-center bg-white p-3 rounded-2xl shadow-sm border border-slate-100 gap-3">
            <div className="relative flex-1 max-w-xs">
              <input type="text" placeholder="Cari tugas..." value={searchFilter} onChange={e => setSearchFilter(e.target.value)}
                className="w-full pl-4 pr-4 py-2 bg-slate-50 border-none rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20" />
            </div>
            <div className="flex gap-3">
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                className="px-4 py-2 bg-white border border-slate-200 rounded-xl text-slate-600 text-sm focus:outline-none focus:border-indigo-500">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <div className="flex items-center gap-3 mb-4 cursor-pointer" onClick={() => setExpanded(e => ({...e, tinggi: !e.tinggi}))}>
              {expanded.tinggi ? <ChevronDown size={20} className="text-slate-400"/> : <ChevronUp size={20} className="text-slate-400"/>}
            ...
            {expanded.tinggi && !loading && <PrioritasTable tasks={tinggi} onView={handleView} onEdit={handleEdit} onDelete={handleDelete} />}
            {loading && <div className="h-24 bg-slate-100 animate-pulse rounded-xl"></div>}
          </div>

          {/* Sedang */}
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <div className="flex items-center gap-3 mb-4 cursor-pointer" onClick={() => setExpanded(e => ({...e, sedang: !e.sedang}))}>
              {expanded.sedang ? <ChevronDown size={20} className="text-slate-400"/> : <ChevronUp size={20} className="text-slate-400"/>}
            ...
            {expanded.sedang && !loading && <PrioritasTable tasks={sedang} onView={handleView} onEdit={handleEdit} onDelete={handleDelete} />}
            {loading && <div className="h-24 bg-slate-100 animate-pulse rounded-xl"></div>}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <div className="flex items-center gap-3 mb-4 cursor-pointer" onClick={() => setExpanded(e => ({...e, rendah: !e.rendah}))}>
              {expanded.rendah ? <ChevronDown size={20} className="text-slate-400"/> : <ChevronUp size={20} className="text-slate-400"/>}
            ...
            {expanded.rendah && !loading && <PrioritasTable tasks={rendah} onView={handleView} onEdit={handleEdit} onDelete={handleDelete} />}
            {loading && <div className="h-24 bg-slate-100 animate-pulse rounded-xl"></div>}
          </div>
        </div>
        ...
        <div className="w-72 space-y-5 shrink-0">
          ...
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
            <h3 className="text-sm font-bold text-slate-800 mb-4">Distribusi Prioritas</h3>
            <div className="flex items-center gap-4">
          ...
          {/* Tips */}
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
            <h3 className="text-sm font-bold text-slate-800 mb-4">Tips Prioritas</h3>
                ...
                return (
                  <div key={t.id} className="flex gap-3 items-center">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-slate-500 truncate">{t.course?.name}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-[11px] font-semibold text-slate-700">{formatDate(t.deadline)}</p>
