...
import { Search, Eye, Edit, Trash2, Plus, AlertCircle, X, ChevronDown } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input type="text" placeholder="Cari tugas..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
            className="w-full pl-4 pr-10 py-2 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-500" />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        {loading ? (
          <div className="space-y-3 p-6">{[1,2,3,4,5].map(i => <div key={i} className="h-12 bg-slate-100 animate-pulse rounded-xl"></div>)}</div>
        ) : paginated.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-slate-400 font-medium">Tidak ada tugas yang ditemukan</p>
          </div>
        ) : (
            ...
            <thead>
              <tr className="border-b border-slate-100">
                {['#','Tugas','Mata Kuliah','Deadline','Status','Progress','Prioritas','Aksi'].map(h => (
                  <th key={h} className="py-4 px-4 font-semibold text-slate-500 text-sm">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {paginated.map((task, i) => (
                <tr key={task.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  <td className="py-4 px-4 text-slate-500 text-sm">{(page - 1) * ITEMS_PER_PAGE + i + 1}</td>
                  <td className="py-4 px-4">
                    <p className="font-bold text-slate-800 text-sm">{task.title}</p>
                    <p className="text-xs text-slate-500">{task.course?.name}</p>
                  </td>
                  <td className="py-4 px-4 text-slate-600 text-sm">{task.course?.name}</td>
                  <td className={`py-4 px-4 font-semibold text-sm ${isOverdue(task.deadline) && task.status !== 'Selesai' ? 'text-red-500' : 'text-slate-600'}`}>{formatDate(task.deadline)}</td>
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-medium text-slate-600 w-8">{task.progress}%</span>
                  ...
                  <td className="py-4 px-4">
                    <div className="flex gap-1">
                      <button onClick={() => { setShowDetailModal(task); setProgressEdit({ progress: task.progress, status: task.status }); }}
                        className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg border border-slate-200 transition-colors"><Eye size={15} /></button>
                      <button onClick={() => { setShowEditModal(task); setEditForm({ title: task.title, course_id: task.course_id, deadline: task.deadline.slice(0,10), priority: task.priority }); }}
                        className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg border border-slate-200 transition-colors"><Edit size={15} /></button>
                      <button onClick={() => setDeleteConfirm(task)}
                        className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg border border-slate-200 transition-colors"><Trash2 size={15} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="flex justify-between items-center text-sm text-slate-500">
          ...
          <div className="flex gap-1">
            <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page===1}
              className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 hover:bg-slate-50 disabled:opacity-40">&lt;</button>
            {Array.from({length: totalPages}, (_, i) => (
              <button key={i} onClick={() => setPage(i+1)}
                className={`w-8 h-8 flex items-center justify-center rounded-lg text-sm font-medium ${page===i+1 ? 'bg-indigo-600 text-white' : 'border border-slate-200 hover:bg-slate-50 text-slate-600'}`}>{i+1}</button>
            ))}
