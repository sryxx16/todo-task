...
import Profil, { getProfileAvatar, PROFILE_UPDATED_EVENT, readStoredProfile, type UserProfile } from "../components/pengaturan/Profil";
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className={`flex items-center gap-3 px-3 py-1.5 rounded-lg shadow-sm border cursor-pointer ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
            <img src={getProfileAvatar(profile)} alt={profile.fullName} className="w-8 h-8 rounded-full object-cover" />
            <span className={`font-medium ${dark ? 'text-slate-200' : 'text-slate-700'}`}>{profile.fullName.split(" ")[0] || profile.username}</span>
